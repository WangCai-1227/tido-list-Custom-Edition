...
| 功能模块 | 说明 |
|:---:|------|
| 🌳 **多级子项目系统** | 无限嵌套子项目（建议 ≤5 层），支持拖拽重组、颜色继承、折叠/展开 |
| 🔍 **智能搜索** | 全文搜索 + 自动展开折叠路径 + 关键词高亮显示 |
| ↩️ **跨路由撤销** | Ctrl+Z 跨 Todo/Done 页面撤销，最多 20 步历史记录 |
| 📋 **级联操作** | 完成父项目自动级联子项目、删除/撤销时级联恢复整棵树 |
| 🎯 **智能颜色系统** | 新建子项目继承父项目颜色，父项目改色时自动同步所有同色子项目 |
| 🎨 **优先级排序** | 高/中/低三级优先级自动排序，文字与编号联动变色 |
| ⚙️ **层级独立配置** | 每级子项目独立设置字号、编号间距、缩进、编号格式 |
| ✏️ **连续删除线** | 子项目完成时，从编号到正文绘制不间断删除线 |
...
- **格式**：`.xlsx`（Excel）
- **包含字段**：序号、内容、优先级、颜色、创建时间、折叠状态/完成时间
- **子项目**：递归导出，层级编号，内容缩进
