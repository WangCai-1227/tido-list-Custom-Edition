<template>
  <div id="app" :class="{ unfocused: ignoreMouse }" :style="appBgStyle">
    <div class="mask"></div>
    <div class="drag-nav">
      <b>{{ leftTitle }}</b>
      <div class="drag-nav-right" :style="dragNavRightStyle">
        <i>{{ rightTitle }}</i>
        <a
          class="brand-link"
          href="#"
          @click.prevent="openWangCai"
          title="打开旺财主页"
          >by 旺财</a
        >
      </div>
    ...
    <div class="nav">
      ...
      <div class="tools">
        <transition-group name="fade" mode="out-in">
          ...
          ></i>
          <span key="export" class="export-wrapper">
            <i
              class="iconfont icon-export"
              @click.stop="importExportShow = !importExportShow"
            ></i>
            <div
              v-if="importExportShow"
              class="import-export-popup"
              @click.stop
            >
      ...
      <div class="settings-content" @click.stop>
        ...
        <div class="settings-body">
            ...
            <div class="section-card">
              <div class="setting-row">
                ...
                <span class="spacing-value"
                  >{{ Math.round(settings.windowOpacity * 100) }}%</span
                >
              </div>
              <div class="setting-row">
                <span class="slider-label">色彩强度</span>
                <input
                  type="range"
                  min="0"
                  max="200"
                  step="5"
                  v-model.number="settings.windowBgStrength"
                  @input="schedulePersistSettings"
                  class="spacing-range"
                />
                <span class="spacing-value"
                  >{{ settings.windowBgStrength }}%</span
                >
              </div>
              <div class="setting-row color-preset-row">
                <span class="slider-label">暗色系</span>
                <div class="preset-color-grid">
                  <button
                    v-for="c in bgDarkColors"
                    :key="c.value"
                    class="strike-color-btn"
                    :class="{ active: settings.windowBgColor === c.value }"
                    :style="{ background: c.value }"
                    @click="
                      settings.windowBgColor = c.value;
              ...
              <div class="setting-row color-preset-row">
                <span class="slider-label">亮色系</span>
                <div class="preset-color-grid">
                  <button
                    v-for="c in bgBrightColors"
                    :key="c.value"
                    class="strike-color-btn"
                    :class="{ active: settings.windowBgColor === c.value }"
                    :style="{ background: c.value }"
              ...
              <div class="setting-row">
                <span class="slider-label">自定义</span>
                <div class="custom-color-row">
                  <input
                    ...
                    @input="
                      settings.windowBgColor = $event.target.value;
                      schedulePersistSettings();
                    "
                  />
                  <input
                    class="hex-input"
                    v-model="settings.windowBgColor"
                    @input="schedulePersistSettings"
                    placeholder="#000000"
                  />
                </div>
          ...
          <div class="settings-section">
            ...
            <div class="section-card">
              <div class="setting-row">
                <span class="slider-label">左侧标题</span>
                <input
                  class="title-input"
              ...
              <div class="setting-row">
                <span class="slider-label">右侧标题</span>
                <input
                  class="title-input"
              ...
              <div class="setting-row">
                <span class="slider-label">链接间距</span>
                <input
                  type="range"
                  min="0"
                  max="30"
                  step="1"
                  v-model.number="settings.linkSpacing"
                  @input="schedulePersistSettings"
                  class="spacing-range"
                />
                <span class="spacing-value">{{ settings.linkSpacing }}px</span>
              </div>
            </div>
          </div>
          ...
          <div class="settings-section">
            ...
            <div class="section-card">
              <div class="setting-row">
                ...
                />
                <span class="spacing-value"
                  >{{ settings.numberSpacing }}px</span
                >
              </div>
          ...
          <div class="settings-section">
            ...
            <div class="level-cards">
              <div
                class="level-card"
                v-for="(label, idx) in levelLabels"
                :key="idx"
              >
                ...
                <div class="level-card-body">
                  <div class="setting-row compact">
                    ...
                    />
                    <span class="spacing-value"
                      >{{ settings.levelConfigs.fontSizes[idx] }}px</span
                    >
                  </div>
                  <div class="setting-row compact">
                    <span class="slider-label">编号间距</span>
                    ...
                    />
                    <span class="spacing-value"
                      >{{ settings.levelConfigs.numberSpacings[idx] }}px</span
                    >
                  </div>
                  <div class="setting-row compact">
                    ...
                    />
                    <span class="spacing-value"
                      >{{ settings.levelConfigs.indents[idx] }}px</span
                    >
                  </div>
                    ...
                    <div class="compact-format-picker">
                      <button
                        ...
                        :class="[
                          ...
                          {
                            active:
                              settings.levelConfigs.formats[idx] === fmt.id
                          }
                        ]"
                        @click="
                          settings.levelConfigs.formats[idx] = fmt.id;
          ...
          <div class="settings-section">
            ...
            <div class="section-card">
              ...
              <div class="setting-row">
                ...
                <div class="strike-color-group">
                  <button
                    ...
                    :style="{
                      background: sc.color || 'transparent',
                      borderColor: sc.color || 'rgba(255,255,255,0.2)'
                    }"
                    @click="
                      settings.strikeColor = sc.value;
                      schedulePersistSettings();
                    "
                    :title="sc.label"
                  >
              ...
              <div
                class="strike-preview"
                :style="{
                  '--strike-color': settings.strikeColor || '#999',
                  '--strike-width': settings.strikeWidth + 'px'
                }"
              >
        ...
        <div class="settings-footer">
          <button class="reset-btn" @click="resetDefaults">恢复默认设置</button>
          <a
            class="footer-link"
            href="#"
...
export default {
  ...
  computed: {
    appBgStyle() {
      const strength = (this.settings.windowBgStrength ?? 100) / 100;
      const rgb = this.hexToRgb(this.settings.windowBgColor);
      const alpha = Math.min(this.settings.windowOpacity * strength, 1);
      return {
        backgroundColor: `rgba(${rgb}, ${alpha})`
      };
    },
    leftTitle() {
      return this.settings.leftTitle || "todo-list";
    },
    rightTitle() {
      return this.settings.rightTitle || "旺财定制版";
    },
    dragNavRightStyle() {
      return {
        gap: (this.settings.linkSpacing ?? 8) + "px"
      };
    },
    groupedFormats() {
    ...
    bgDarkColors() {
      return [
        ...
        { value: "#0d1117", label: "GitHub暗" },
        { value: "#1a1b2e", label: "深空蓝" },
        { value: "#2d1b2e", label: "暗紫" },
        { value: "#1b2d1e", label: "暗绿" },
        { value: "#2d2d1b", label: "暗棕" },
        { value: "#3d1f1f", label: "暗红" },
        { value: "#1f2d3d", label: "钢蓝" },
        { value: "#2a1f3d", label: "皇室紫" },
        { value: "#1f3d2a", label: "森林绿" },
        { value: "#3d3d1f", label: "橄榄" },
        { value: "#4a2a2a", label: "栗色" },
        { value: "#2a3a4a", label: "雾霾蓝" },
        { value: "#2a1a1a", label: "暖灰" }
      ];
    },
    bgBrightColors() {
      return [
        { value: "#dc2626", label: "正红" },
        { value: "#ea580c", label: "橙色" },
        { value: "#ca8a04", label: "金黄" },
        { value: "#16a34a", label: "草绿" },
        { value: "#0891b2", label: "青色" },
        { value: "#2563eb", label: "亮蓝" },
        { value: "#7c3aed", label: "亮紫" },
        { value: "#db2777", label: "玫红" },
        { value: "#0d9488", label: "青绿" },
        { value: "#1d4ed8", label: "深蓝" },
        { value: "#b91c1c", label: "深红" },
        { value: "#a21caf", label: "紫红" },
        { value: "#0369a1", label: "天蓝" },
        { value: "#15803d", label: "深绿" }
      ];
    },
    strikeColorOptions() {
      return [
  ...
  mounted() {
    ...
    document.addEventListener("click", this.closeImportExport);
  ...
  methods: {
    syncSettingsFromDb() {
      ...
      if (
        ...
        typeof this.settings.levelConfigs !== "object"
      ) {
        this.settings.levelConfigs = {
          fontSizes: [16, 14, 13, 12, 12],
          numberSpacings: [6, 4, 4, 4, 4],
      ...
      if (
        !this.settings.levelConfigs.formats ||
        !Array.isArray(this.settings.levelConfigs.formats)
      ) {
        this.settings.levelConfigs.formats = [
          "arabic",
          "letter_lower",
    ...
    persistSettingsNow() {
      ...
      DB.set(
        ...
        Object.assign({}, base, {
          ...
          windowOpacity: this.settings.windowOpacity,
          windowBgColor: this.settings.windowBgColor,
          windowBgStrength: this.settings.windowBgStrength,
          leftTitle: this.settings.leftTitle,
          rightTitle: this.settings.rightTitle,
          linkSpacing: this.settings.linkSpacing,
    ...
    importData() {
      this.importExportShow = false;
      ipcRenderer.invoke("importData");
    },
    openWangCai() {
      ipcRenderer.invoke("openExternal", "https://github.com/WangCai-1227");
    },
    openProjectUrl() {
      ipcRenderer.invoke(
        "openExternal",
        "https://github.com/WangCai-1227/tido-list-Custom-Edition"
      );
    },
    resetDefaults() {
      this.settings = {
      ...
      this.currentFormat = "arabic";
      this.persistSettingsNow();
      DB.set("numberFormat", "arabic");
    },
    hexToRgb(hex) {
      if (!hex) return "0,0,0";
      const c = hex.replace("#", "");
  ...
  beforeDestroy() {
    this.flushPersistSettings();
    document.removeEventListener("click", this.closeImportExport);
