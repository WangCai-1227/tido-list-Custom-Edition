<template>
  <div class="root">
    ...
    <div
      class="list"
      v-for="(group, gIdx) in displayList"
      :key="gIdx"
    >
      ...
      >
        <div class="item-body" :class="{ 'done-child': entry.item.isChildDone }" :style="strikeColorStyle(entry)">
...
export default {
  name: "Done",
  inject: ["getNumberFormat", "getNumberVisible", "getChildNumberVisible", "getLevelConfigs", "getStrikeWidth", "getStrikeColor"],
  data() {
    return {
      doneGroupList: null,
      searchQuery: "",
  ...
  methods: {
    ...
    strikeColorStyle(entry) {
      ...
      const useColor = this.strikeColor || this.doneAccentColor(entry.item) || "#999";
      return {
    ...
    flattenTree(items) {
      ...
      const walk = (arr, depth) => {
        arr.forEach(item => {
          ...
          if (item.children && item.children.length > 0 && !this.isCollapsed(item)) {
  ...
  computed: {
    ...
    displayList() {
      ...
      for (const key in groups) {
        ...
        if (q) {
          filtered = enriched.filter(e => e.item.content.toLowerCase().includes(q));
        }
