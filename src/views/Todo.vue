    ...
    <draggable
      ...
      >
        <transition-group type="transition" :name="!drag ? 'flip-list' : null">
          ...
          >
            <span v-if="hasChildren(entry.item) && entry._depth > 0" class="child-collapse-spacer"></span>
            <div class="item-body" :class="{ 'done-child': entry.item.isChildDone }" :style="itemBodyStyle(entry)">
              <span class="number" :style="numberSpanStyle(entry)">{{ getNumber(entry) }}</span>
              <p
              ...
              <div v-else class="edit" @click.stop>
                ...
                <div class="edit-tools">
                  ...
                  <div class="priority-editor priority-popover">
                    ...
                    >
                      <button
                        ...
                        :class="{ active: (entry.item.priority || null) === option.value }"
                  ...
                  <i class="iconfont icon-close" @click.stop="remove(entry.item)"></i>
                </div>
              </div>
            </div>
            <span
...
export default {
  ...
  inject: ["getNumberFormat", "getNumberSpacing", "getNumberVisible", "getChildNumberVisible", "getLevelConfigs", "getStrikeWidth", "getStrikeColor"],
  data() {
    return {
      todoList: null,
      dragList: [],
  ...
  methods: {
    ...
    ensureItemReactive(items) {
      ...
      for (const item of items) {
        ...
        this.$set(item, "priority", item.priority != null ? item.priority : null);
    ...
    onDragEnd(evt) {
      ...
      this.$nextTick(() => {
        if (movedEntry) {
          const el = this.$el.querySelector(`[data-item-id="${movedEntry.item.id}"]`);
          if (el) {
    ...
    numberSpanStyle(entry) {
      ...
      const spacing = isChild ? this.levelNumberSpacing(depth) : this.numberSpacing;
    ...
    itemBodyStyle(entry) {
      ...
      const useColor = this.strikeColor || this.itemAccentColor(entry.item) || "#999";
      return {
    ...
    levelIndentTotal(depth) {
      ...
      for (let i = 0; i < depth; i++) {
        total += i < arr.length ? arr[i] : (arr.length > 0 ? arr[arr.length - 1] : 18);
      }
      return total;
    },
    levelFormat(depth) {
    ...
    getNumber(entry) {
      ...
      const fmt = entry._depth > 0 ? this.levelFormat(entry._depth) : this.formatPattern;
    ...
    cascadeDone(todo) {
      ...
      const itemsToDone = [];
      const collect = (node) => {
        itemsToDone.push(Object.assign(
          { done_date: getNowDate(), done_datetime: getNowDateTime() },
          DB.deepClone(node)
        ));
    ...
    handleUndo() {
      ...
      if (action.type === "done") {
        this.todoList.push(Object.assign({}, action.item));
        ...
        this.todoList.push(Object.assign({}, action.items[0]));
        DB.set("todoList", this.todoList);
        EventBus.$emit("refresh-done");
        this.$nextTick(() => this.syncDragList());
  ...
  computed: {
    sortedTodoList() {
      ...
      const hasPriority = this.todoList.some(item => item.priority != null);
      if (!hasPriority) {
      ...
      const rank = priority => {
        ...
        return priorityOrder[priority] != null ? priorityOrder[priority] : 3;
      };
    ...
    levelConfigs() {
      ...
      return (typeof fn === "function" ? fn() : null) || { fontSizes: [], numberSpacings: [], indents: [] };
    },
    formatPattern() {
